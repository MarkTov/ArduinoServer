...
(0, _axiosDefault.default).defaults.baseURL = "http://localhost:3001";
const getData = async function() {
    const data = await (0, _axiosDefault.default).get("/data");
    console.log(data);
    return data.data;
};
getData();
function createCard(name, temperature, humidity) {
    const div = `<div class="card">
    <h2>${name}</h2>
    <p class="temperature">Temperature: <span class="data">${temperature}</span></p>
    <p class="humidity">Humidity: <span class="data">${humidity}</span></p>
</div>`;
    return div;
}
async function createTable() {
    ...
    let line = "";
    for(let i = 0; i < data.length; i++)line = line + createCard(data[i].name, data[i].temperature, data[i].humidity);
    return line;
}
const table = document.querySelector(".table");
async function main() {
    table.innerHTML = await createTable();
}
main();
setInterval(()=>{
    main();
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["icZzK","8lqZg"], "8lqZg", "parcelRequiref59e")
